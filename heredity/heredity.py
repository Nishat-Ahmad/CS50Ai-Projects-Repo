...
def main():
    ...
    people = load_data(sys.argv[1]) # Stores data from csv file
...
def copies(person, one_gene, two_genes):
...
def prob_no_parents(copies_gene, has_trait):
    return (PROBS["gene"][copies_gene] * PROBS["trait"][copies_gene][has_trait])

def prob_parent(person, people, one_gene, two_genes, have_trait):
    # If has mom and dad, get their genes
    # Get name of the mom
    mother = people[person]['mother']
    # Check how many genes she has
    mother_genes = copies(mother, one_gene, two_genes)
    # Get name of the dad
    father = people[person]['father']
    # Check how many genes she has
    father_genes = copies(father, one_gene, two_genes)

    childGene = copies(person, one_gene, two_genes)
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        copies_gene = copies(person, one_gene, two_genes)
        ...
        if people[person]["mother"] == None:
            prob *= prob_no_parents(copies_gene, has_trait)
        else:
            prob *= (prob_parent(person, people, one_gene, two_genes, have_trait) * PROBS["trait"][copies_gene][has_trait])
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        total_pGenes = sum(probabilities[person]["gene"].values())
        total_pTrait = sum(probabilities[person]["trait"].values())
        
        for gene in probabilities[person]["gene"]:
            original_value = probabilities[person]["gene"][gene]
            new_value = original_value / total_pGenes
            probabilities[person]["gene"][gene] = new_value
        
        for trait in probabilities[person]["trait"]:
            original_value = probabilities[person]["trait"][trait]
            new_value = original_value / total_pTrait
